...
- **Inflight** markers detect interrupted turns; the next `resume` continues the same session slot with a reconciliation prompt. A resume that continues prints the phase, session, turn, and `Status: RUNNING`.
- **Approved history** — With `git.protect_approved_history`, the approved commit must stay an ancestor of HEAD. If a rewrite removes it, auto-loop remaps `last_approved_commit` and matching `approved_evidence` only when one ancestor of HEAD has the same product tree. The `history_reconciliation` event records the old SHA, the new SHA, the evidence, and HEAD at that time. Later commits stay unapproved until review. If the mapping is missing or ambiguous, the baseline is left unchanged, the run stops with `GIT_PROTOCOL_ERROR`, and `auto-loop status` reports `NEEDS HISTORY RECONCILIATION`. Resolve that with `auto-loop reconcile-history RUN_CONFIG --approved <candidate-sha>` (only against the persisted reconciliation record; ancestry and product-tree evidence are verified before trust state updates).
