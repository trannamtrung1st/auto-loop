...
- After a **terminal** run (`completion.json` or `blocked.json`), supplying a **new goal** archives the prior run’s plan, context, task snapshot, reviews, and runtime notes under `.auto-loop/runtime/archives/<lifecycle-id>/` using each file’s workspace-relative path (so custom paths with the same basename do not collide). Default **plan** and **context** are then regenerated (custom agent/instruction files are preserved). Active (non-terminal) runs must be resumed, not replaced. `auto-loop resume` (or `run` with no new goal) on a blocked terminal run exits with `BLOCKED` and the saved summary rather than re-entering the loop.
